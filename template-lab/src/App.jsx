...
export default function App() {
  ...
  // Parse ?guest=Name — supports both + and %20 for spaces
  const params = new URLSearchParams(window.location.search);
  const guestName = params.get('guest') || '';

  // Feature #15: Track card open
  useEffect(() => {
    if (loaderDone) trackOpen(guestName);
  }, [loaderDone, guestName]);
  ...
  return (
    <div className="relative w-full bg-[#F8F7F4]">
      ...
      {/* Scroll container — order: Muhurtham → Venue → Reception → Gallery → Family → RSVP */}
